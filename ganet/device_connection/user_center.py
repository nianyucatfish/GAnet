...
async function copySetupPrompt(){const value='帮我配置设备互联';try{await navigator.clipboard.writeText(value)}catch(e){const t=document.createElement('textarea');t.value=value;document.body.appendChild(t);t.select();document.execCommand('copy');t.remove()}document.getElementById('copyHint').textContent='已复制，可以直接发送给 GA。'}
